          ...
          <Reveal key={project.title} direction={index % 2 === 0 ? 'up' : 'scale'} delay={160 + index * 110}>
            <article className="glass-panel flex flex-col overflow-hidden">
              <div className="relative overflow-hidden rounded-[1.5rem]">
                ...
                />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_35%,rgba(7,25,51,0.5)_100%)]" />
              </div>

              <div className="flex flex-col p-6">
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="rounded-full border border-white/16 bg-white/10 px-3 py-1 text-xs uppercase tracking-[0.16em] text-blue-50/80">
                      {tag}
                    </span>
                  ))}
                </div>
